...
function buildDevices() {
  ...
  return raw.map((d) => {
    ...
    if (d.type === 'dynamips' && routers.has(d.name)) {
      ...
      if (hosts.has(d.name)) communication = hosts.get(d.name).reachable ? 'Reachable' : 'Unreachable';
      if (d.running) {                       // simulate so VPCS show up in gauges/charts
...
module.exports = {
  ...
  overview() {
    ...
    return {
      ...
      deviceIps: up.filter((d) => d.type !== 'ethernet_switch').map((d) => ({ name: d.hostname, ip: d.ip })),
