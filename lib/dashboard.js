...
function buildDevices() {
  ...
  return raw.map((d) => {
    let cpu = null, ram = null, disk = null, sent = null, received = null;
    let communication = d.running ? 'Reachable' : 'Unreachable';

    if (d.type === 'dynamips' && routers.has(d.name)) {
      const r = routers.get(d.name);
      cpu = r.cpu5sec; ram = r.memUsedPct; disk = null; // routers expose flash, not a disk %
      const inK = (r.interfaces || []).reduce((s, i) => s + (i.inKbps || 0), 0);
      const outK = (r.interfaces || []).reduce((s, i) => s + (i.outKbps || 0), 0);
      received = +(inK / 1000).toFixed(1); sent = +(outK / 1000).toFixed(1);
    } else if (d.type === 'qemu' && linux.has(d.name)) {
      const l = linux.get(d.name);
      cpu = l.cpuPct; ram = l.memUsedPct; disk = l.diskUsedPct;
      sent = l.txKbps == null ? null : +(l.txKbps / 1000).toFixed(1);
      received = l.rxKbps == null ? null : +(l.rxKbps / 1000).toFixed(1);
    ...
    return {
      id: d.id, type: d.type,
      ip: d.ip || '—', hostname: d.name, osName: osLabel(d.type),
...
function tick() {
  const p = probe.data();
  if (!p.lastSweep || p.lastSweep === lastSweepSeen) return;
  lastSweepSeen = p.lastSweep;
  const a = aggregate(buildDevices());
  history.push({ t: p.lastSweep, ...a });
  while (history.length > HISTORY_LEN) history.shift();
}

module.exports = {
  start() { setInterval(tick, 3000); },
  ...
  overview() {
    ...
    return {
      ...
      deviceIps: up.map((d) => d.ip).filter((ip) => ip && ip !== '—'),
