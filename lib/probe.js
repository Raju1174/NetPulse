...
// ---- background sweep -------------------------------------------------------
const cache = { hosts: [], routers: [], sweeping: false, lastSweep: 0, error: null };
...
async function sweep(getDevices) {
  ...
  try {
    ...
    // routers first (most valuable), then hosts; sequential to be gentle on consoles
    const rOut = [];
...
if (require.main === module) {
  ...
  (kind === 'router' ? probeRouter(dev) : probeHost(dev)).then((r) => {
