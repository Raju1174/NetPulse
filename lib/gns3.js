...
function shape(nodes, links, configs) {
  ...
  // 2) group into subnets. A host's subnet comes from its own config, or is
  //    inferred from the switch (and the router interface) it connects to — so
  //    renamed VMs with no config still land in the right network.
  const NETLABELS = {
    '192.168.1.0/24': 'Client LAN',
    '192.168.3.0/26': 'Middle Tier (DMZ)',
    '10.10.10.0/24': 'Router Interconnect',
    '10.10.20.0/24': 'Server Farm',
  };
  const labelFor = (net) => NETLABELS[net] || `Subnet ${net}`;
  const isHost = (t) => t === 'vpcs' || t === 'qemu' || t === 'docker';
  const nodeType = new Map(nodes.map((n) => [n.node_id, n.node_type]));
  ...
  // router port -> subnet, then switch -> subnet, then host -> switch's subnet
  const portSubnet = new Map();
  for (const d of devices) {
    if (d.type !== 'dynamips') continue;
    for (const i of d.interfaces) {
      const m = i.ip && i.ip !== 'dhcp' && i.cidr != null && i.iface.match(/(\d+)\/(\d+)/);
      if (m) portSubnet.set(`${d.id}:${m[1]}:${m[2]}`, { network: `${networkAddr(i.ip, i.cidr)}/${i.cidr}`, mask: cidrToMask(i.cidr), gateway: i.ip });
    }
  }
  const switchSubnet = new Map();
  for (const l of links) {
    for (const [x, y] of [[l.nodes[0], l.nodes[1]], [l.nodes[1], l.nodes[0]]]) {
      if (!x || !y) continue;
      const s = portSubnet.get(`${x.node_id}:${x.adapter_number}:${x.port_number}`);
      if (s && nodeType.get(y.node_id) === 'ethernet_switch') switchSubnet.set(y.node_id, s);
    }
  }
  const hostSubnet = new Map();
  for (const l of links) {
    for (const [x, y] of [[l.nodes[0], l.nodes[1]], [l.nodes[1], l.nodes[0]]]) {
      if (!x || !y) continue;
      if (switchSubnet.has(x.node_id) && isHost(nodeType.get(y.node_id))) hostSubnet.set(y.node_id, switchSubnet.get(x.node_id));
    }
  }

  for (const d of devices) {
    if (!isHost(d.type)) continue;
    let net = d.network, mask = d.subnet, gw = d.gateway;
    if (!net && hostSubnet.has(d.id)) { const s = hostSubnet.get(d.id); net = s.network; mask = s.mask; gw = s.gateway; }
    if (!net) continue;
    const g = ensure(net, labelFor(net));
    g.subnet = mask;
    g.devices.push({ ...d, isRouter: false });
    if (gw && !g.gatewayIp) g.gatewayIp = gw;
  }
  ...
  if (infra.length) {
    const g = ensure('infrastructure', 'Infrastructure');
